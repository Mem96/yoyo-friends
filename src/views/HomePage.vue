...
export default {
  ...
  methods: {
    getVideos() {
      axios.get("https://ott-fogliata.github.io/vuejs-s2i-repository/yoyo.json")
      .then(result => this.videosList = result.data)
      .catch(e => {console.log(e); alert('An error has occured.')})
    },
    filterList(){
      ...
      if (this.searchedItem != ''){
        ...
        for (let i in this.videosList){
          ...
          if (searchedArr.every(word => videoSpecs.includes(word))){
            if (!this.showOnSearch.includes(this.videosList[i])){ // avoids repetition
